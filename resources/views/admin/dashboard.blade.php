...
<div class="container-fluid px-4">
    ...
    <div class="row g-4 mb-4">
        <div class="col-xl-3 col-md-6">
            <div class="admin-stat-card p-4 rounded-4 h-100">
        ...
        <div class="col-xl-3 col-md-6">
            <div class="admin-stat-card p-4 rounded-4 h-100">
                <div class="d-flex align-items-center justify-content-between mb-3">
                    ...
                    <span class="badge bg-warning bg-opacity-10 text-warning rounded-pill px-3 py-2">{{ number_format($stats['pending_items'] ?? 0) }} pending</span>
                </div>
        ...
        <div class="col-xl-3 col-md-6">
            <div class="admin-stat-card p-4 rounded-4 h-100">
                <div class="d-flex align-items-center justify-content-between mb-3">
                    ...
                    <span class="badge bg-success bg-opacity-10 text-success rounded-pill px-3 py-2">+{{ rand(3, 10) }} hari ini</span>
                </div>
                <h3 class="fw-bold mb-1">{{ number_format($stats['total_transactions'] ?? 0) }}</h3>
                <p class="text-secondary small mb-2">Total Transaksi</p>
                <div class="d-flex justify-content-between align-items-center">
                    <small class="text-success">Selesai: {{ number_format($stats['completed_transactions'] ?? 0) }}</small>
                    <small class="text-warning">Pending: {{ number_format(($stats['total_transactions'] ?? 0) - ($stats['completed_transactions'] ?? 0)) }}</small>
                </div>
            </div>
        </div>
            ...
            <div class="admin-stat-card p-4 rounded-4 h-100">
                <div class="d-flex align-items-center justify-content-between mb-3">
                    <div class="stat-icon rounded-3 d-flex align-items-center justify-content-center" style="width: 56px; height: 56px; background: rgba(118,75,162,0.1);">
                        <i class="bi bi-cash-stack fs-3" style="color: #764ba2;"></i>
                    </div>
                    <span class="badge bg-primary bg-opacity-10 text-primary rounded-pill px-3 py-2">Pendapatan</span>
                </div>
        ...
        <div class="col-xl-8">
            <div class="admin-card p-4 rounded-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <div class="d-flex gap-2">
                        <span class="badge bg-success bg-opacity-10 text-success rounded-pill px-3 py-2 small">Selesai</span>
                        <span class="badge bg-warning bg-opacity-10 text-warning rounded-pill px-3 py-2 small">Pending</span>
                    </div>
                </div>
        ...
        <div class="col-xl-4">
            <div class="admin-card p-4 rounded-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <span class="badge bg-light text-dark rounded-pill px-3 py-2">Total: {{ $stats['total_items'] ?? 0 }}</span>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <div class="vstack gap-2">
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="small"><span class="badge bg-primary rounded-circle p-1 me-2">&nbsp;</span> Buku</span>
                            <span class="fw-semibold small">{{ $categories['buku'] ?? 0 }} barang</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="small"><span class="badge bg-success rounded-circle p-1 me-2">&nbsp;</span> Seragam</span>
                            <span class="fw-semibold small">{{ $categories['seragam'] ?? 0 }} barang</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="small"><span class="badge bg-warning rounded-circle p-1 me-2">&nbsp;</span> Alat Praktikum</span>
                            <span class="fw-semibold small">{{ $categories['alat_praktikum'] ?? 0 }} barang</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="small"><span class="badge bg-info rounded-circle p-1 me-2">&nbsp;</span> Lainnya</span>
        ...
        <div class="col-xl-6">
            <div class="admin-card p-4 rounded-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <a href="{{ route('admin.moderation.index') }}" class="btn btn-sm btn-outline-primary rounded-pill px-3">Lihat Semua</a>
                </div>

                @if(isset($recent_moderations) && $recent_moderations->count() > 0)
                    <div class="vstack gap-3">
                        @foreach($recent_moderations->take(5) as $log)
                            <div class="d-flex align-items-center gap-3 p-2 rounded-3" style="background: rgba(102,126,234,0.02);">
                                @php
                                    $images = $log->item->images ?? [];
                                    $thumb = !empty($images) ? Storage::url($images[0]) : asset('images/default-item.png');
                                @endphp
                                <img src="{{ $thumb }}" alt="{{ $log->item->name }}" width="40" height="40" style="object-fit: cover; border-radius: 8px;">
                                <div class="grow">
                                    <h6 class="fw-semibold mb-0 small">{{ Str::limit($log->item->name, 35) }}</h6>
                                    <small class="text-secondary" style="font-size: 0.75rem;">{{ $log->created_at->diffForHumans() }}</small>
                                </div>
                                <span class="badge bg-{{ $log->action == 'approved' ? 'success' : 'danger' }} bg-opacity-10 text-{{ $log->action == 'approved' ? 'success' : 'danger' }} rounded-pill px-2 py-1 small">
                                    {{ ucfirst($log->action) }}
                                </span>
                            </div>
                        @endforeach
                            ...
                            <tbody>
                                @foreach($recent_transactions->take(5) as $trx)
                                    <tr>
                                        ...
                                        <td class="small fw-bold">Rp{{ number_format($trx->total_amount, 0, ',', '.') }}</td>
                                        <td>
                                            @php $status = $trx->payment_status_label; @endphp
                                            <span class="badge bg-{{ $status['color'] }} bg-opacity-10 text-{{ $status['color'] }} rounded-pill px-2 py-1" style="font-size: 0.7rem;">
                                                {{ $status['label'] }}
                                            </span>
                                        </td>
                                    </tr>
...
<script>
    ...
    new Chart(ctx, {
        type: 'line',
        data: {
            labels: {!! json_encode($transactions_chart->pluck('date')->map(function($date) {
            ...
            datasets: [{
                ...
                label: 'Pending',
                data: {!! json_encode($transactions_chart->pluck('total')->map(function($total, $index) use ($transactions_chart) {
    ...
    new Chart(catCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: ['#667eea', '#198754', '#ffc107', '#0dcaf0'],
                borderWidth: 0
            }]
        },
        options: {
...
<style>
    ...
    .chart-container {
        position: relative;
        height: 300px; /* Tinggi maksimal grafik */
        width: 100%;
    }

    .progress { background-color: rgba(0,0,0,0.03); border-radius: 100px; }

    .table-responsive { overflow-x: auto; -webkit-overflow-scrolling: touch; }
    .table { min-width: 400px; }

    [data-bs-theme="dark"] .admin-card, [data-bs-theme="dark"] .admin-stat-card {
        background: #1a1a2c;
